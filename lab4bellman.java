...
public class lab4bellman{
        public static void main(String[] args){
                ...
                bellman(n,graph,source-1);
                sc.close();
        }
        public static void bellman(int n,int graph[][],int source){
                int dist[]=new int[n];
                ...
                for(int u=0;u<n;u++){
                        for(int v=0;v<n;v++){
                                if(graph[u][v]!=0 && dist[u]!=Integer.MAX_VALUE && dist[u]+graph[u][v]<dist[v]){
                                        System.out.println("Negative weight cycle deteced,");
                                        return;
                                }
                        }
                }
